...
namespace ShopQuanAo
{
    public class XLhinhanh_Vantay
    {
		...
		public XLhinhanh_Vantay()
		{
		}
		public XLhinhanh_Vantay(string sourceFile,int widthSquare)
		{
        ...
        public XLhinhanh_Vantay(int[,] inputImage, int inputWidth, int inputHeight, int widthSquare)
		{
			image = inputImage;
			width = inputWidth;
			height = inputHeight;
		...
		{		
			MaskGabor mask;
			for(int i=0;i<KiemtraVanTay.maskNumber;i++)
			{
				if(filterDirect>=i*Math.PI/KiemtraVanTay.maskNumber&&filterDirect<(i+1)*Math.PI/KiemtraVanTay.maskNumber)
				{
					mask = (MaskGabor) KiemtraVanTay.MaskGaborCollection[i];
					return mask.Mask;
				}
			}
			mask = (MaskGabor)KiemtraVanTay.MaskGaborCollection[KiemtraVanTay.maskNumber-1];
			return mask.Mask;
